...
class TL45MCCodeEmitter : public MCCodeEmitter {
  ...
  unsigned getJmpTargetHOpValue(const MCInst &MI, unsigned OpNo,
                                SmallVectorImpl<MCFixup> &Fixups,
                                const MCSubtargetInfo &STI) const;
...
unsigned TL45MCCodeEmitter::getImmOpValue(const MCInst &MI, unsigned OpNo,
  ...
  if (Kind == MCExpr::Target) {
    ...
    if (MI.getOpcode() == TL45::ADDHI || MI.getOpcode() == TL45::LdAH) {
...
unsigned TL45MCCodeEmitter::getJmpTargetLOpValue(const MCInst &MI, unsigned OpNo, SmallVectorImpl<MCFixup> &Fixups,
                                                 const MCSubtargetInfo &STI) const {
  // bool EnableRelax = STI.getFeatureBits()[TL45::FeatureRelax];
  const MCOperand &MO = MI.getOperand(OpNo);
...
unsigned TL45MCCodeEmitter::getJmpTargetHOpValue(const MCInst &MI, unsigned OpNo, SmallVectorImpl<MCFixup> &Fixups,
                                                 const MCSubtargetInfo &STI) const {
  const MCOperand &MO = MI.getOperand(OpNo);
